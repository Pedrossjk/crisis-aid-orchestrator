...
import { Home, Map, Bell, User, LayoutDashboard, Users, Package, HeartHandshake, Sparkles } from "lucide-react";
...
const navsByRole: Record<Role, { to: string; label: string; icon: typeof Home }[]> = {
  volunteer: [
    { to: "/volunteer", label: "Início", icon: Home },
  ...
  "ong-need": [
    { to: "/ong-need", label: "Painel", icon: LayoutDashboard },
    { to: "/ong-need/volunteers", label: "Voluntários", icon: Users },
    { to: "/ong-need/requests", label: "Solicitações", icon: HeartHandshake },
    { to: "/ong-need/profile", label: "Perfil", icon: User },
  ],
  "ong-offer": [
    { to: "/ong-offer", label: "Painel", icon: LayoutDashboard },
    { to: "/ong-offer/resources", label: "Recursos", icon: Package },
    { to: "/ong-offer/matches", label: "Matches IA", icon: Sparkles },
    { to: "/ong-offer/profile", label: "Perfil", icon: User },
  ],
};
...
export function AppShell({ role, children }: { role: Role; children: React.ReactNode }) {
  ...
  const roleMeta = {
    ...
    "ong-need": { label: "ONG · Precisa de ajuda", color: "bg-success" },
    "ong-offer": { label: "ONG · Oferece ajuda", color: "bg-ai" },
  }[role];
      ...
      <header className="sticky top-0 z-40 glass-card border-b border-border/60">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/volunteer/notifications" className="relative rounded-lg p-2 hover:bg-muted transition">
