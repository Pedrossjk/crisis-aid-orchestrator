...
import { actions, matchedVolunteers, ngoConnections, helpTypeLabels, urgencyLabels, type Urgency, type Volunteer, type NgoConnection } from "@/lib/mock-data";
import { Plus, Sparkles, Users, CheckCircle2, Clock, AlertCircle, Star, MapPin, ArrowRight, Package, Network, ListChecks, Send, Loader2, Building2, UserCheck, X } from "lucide-react";
...
const initialPendingRequests: PendingRequest[] = [
  { id: "pr1", org: "Saúde Sem Fronteiras", orgInitials: "SF", city: "Florianópolis, SC", topic: "Equipe médica", message: "Olá! Identificamos que vocês possuem recursos compatíveis com nossa necessidade urgente de equipe médica para triagem em abrigo. Gostaríamos de estabelecer uma parceria.", receivedAgo: "há 2h", matchScore: 88 },
  { id: "pr2", org: "Lar dos Pequenos", orgInitials: "LP", city: "Joinville, SC", topic: "Alimentos e cobertores", message: "Prezados, nossa organização está atendendo 120 crianças desabrigadas e precisamos de apoio com alimentos e cobertores. Podemos colaborar?", receivedAgo: "há 5h", matchScore: 79 },
];
...
function OngDashboard() {
  ...
  const [invite, setInvite] = useState<InviteState>({ volunteer: null, message: "", sending: false, sent: false });
  ...
  const openInvite = (v: Volunteer) => {
    setInvite({ volunteer: v, message: buildInviteMessage(ongName, v), sending: false, sent: false });
  };
  ...
  return (
    <AppShell role="ong">
            ...
            {matchedVolunteers.slice(0, 3).map((v) => (
              <div key={v.id} className="flex items-center gap-3 border border-border/60 bg-card p-4 shadow-soft">
                ...
                {invitedVols.has(v.id) ? (
                  ...
                  <Button size="sm" variant="outline" onClick={() => openInvite(v)}>Convidar</Button>
                )}
              </div>
            ))}
          </div>
      ...
      {/* Invite dialog */}
      <Dialog open={!!invite.volunteer} onOpenChange={(open) => { if (!open) closeInvite(); }}>
        <DialogContent className="max-w-lg">
          ...
          {invite.sent ? (
            <div className="py-6 text-center space-y-3">
              ...
              <Button className="w-full mt-2" onClick={closeInvite}>Fechar</Button>
            </div>
          ) : (
            <>
                ...
                {invite.volunteer && (
                  <div className="flex items-center gap-3 rounded-xl bg-muted p-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{invite.volunteer.skills.join(" · ")} · {invite.volunteer.distanceKm}km</p>
                    </div>
                    <span className="ml-auto rounded-full bg-ai/10 px-2 py-0.5 text-[10px] font-bold text-ai">{invite.volunteer.matchScore}% match</span>
                  </div>
                )}
                <div>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={closeInvite} disabled={invite.sending}>Cancelar</Button>
                <Button onClick={sendInvite} disabled={invite.sending || !invite.message.trim()} className="bg-gradient-hero">
                  {invite.sending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}
                  Enviar convite
                </Button>
              </DialogFooter>
            </>
      ...
      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Cooperação direta entre instituições</p>
          </div>
            ...
            {activeConnections.length === 0 && localAccepted.length === 0 ? (
              ...
              <div className="space-y-2">
                {[...activeConnections, ...localAccepted].slice(0, 4).map((c) => (
                  ...
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-ai text-ai-foreground font-bold text-sm">{c.orgInitials}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate">{c.org}</p>
                      <p className="text-xs text-muted-foreground truncate">{c.topic} · {c.city}</p>
                    </div>
                    {c.unread && c.unread > 0 ? (
                      <span className="rounded-full bg-primary text-primary-foreground px-1.5 py-0.5 text-[10px] font-bold">{c.unread}</span>
                    ) : (
          ...
          <div>
            <div className="mb-3 flex items-center gap-2">
              ...
              {localPending.length > 0 && (
                <span className="rounded-full bg-warning/15 px-1.5 py-0.5 text-[10px] font-bold text-warning">{localPending.length}</span>
              )}
            </div>
            {localPending.length === 0 ? (
              <div className="rounded-2xl border border-border/60 bg-muted/40 p-6 text-center text-sm text-muted-foreground">
                ...
                {localPending.map((r) => (
                  <div key={r.id} className="rounded-2xl border border-border/60 bg-card p-4 shadow-soft">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-ai text-ai-foreground font-bold text-sm">{r.orgInitials}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold">{r.org}</p>
                        <p className="text-xs text-muted-foreground truncate">{r.city} · {r.topic}</p>
                      </div>
                      <span className="text-[10px] text-muted-foreground shrink-0">{r.receivedAgo}</span>
                    </div>
                    <p className="mt-2 text-xs text-muted-foreground italic bg-muted/40 rounded-lg px-3 py-2 line-clamp-2">
                      "{r.message}"
                    </p>
                    <div className="mt-3 flex gap-2">
                      <Button variant="outline" size="sm" className="text-xs gap-1" onClick={() => setProfileNgo(r)}>
          ...
          {profileNgo && (
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-border/60 bg-muted/30 p-4 space-y-3">
                ...
                <div>
                  <p className="text-[11px] font-medium uppercase text-muted-foreground">Compatibilidade da IA</p>
